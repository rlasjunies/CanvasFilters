module mtg.util.canvas {
    export interface IDataImageFloat32 {
        width: number;
        height: number;
        data: Float32Array;
    }
    ...
    export class Canvas {
        ...
        private getPixels( img: HTMLImageElement ) {
        ...
        brightness( adjustment?: number ) {
            if ( adjustment == undefined ) adjustment = 40;
            //var d = pixels.data;
            var d = this.imagedata.data;
            for ( var i = 0; i < d.length; i += 4 ) {
                d[i] += adjustment;
        ...
        convoluteFloat32( weights, opaque ): IDataImageFloat32 {
            var side = Math.round( Math.sqrt( weights.length ) );
            var halfSide = Math.floor( side / 2 );
        ...
        mergeSobel( src1: IDataImageFloat32, src2: IDataImageFloat32  ) {
